...
/**
 * java.nio（java non-blocking IO）,java非阻塞式IO。 特 性 Channel,Buffer,Selector 目 的 提供非阻塞式的高伸缩性网络。 Channel 有点象流，
...
public class NonBlockingDemo {
    ...
    public static void main(String[] args) throws Exception {
        MappedFile file = new MappedFile("F:/mysql/test/aa.txt");
        if(file.boundChannelToByteBuffer()){
            for(int i=0;i<10_0000;i++){
                file.appendData("java.nio（java non-blocking IO）,java非阻塞式IO。 特 性 Channel,Buffer,Selector 目 的 提供非阻塞式的高伸缩性网络。 Channel 有点象流， 数据可以从Channel读到Buffer中，也可以从Buffer 写到Channel中。\n".getBytes(StandardCharsets.UTF_8));
            }
        }
        ;
    }
