 ...
 * <p>
 * 实现原理，Base64实际上是对二进制码做分组转换操作
 * <li>1. 每3个8位二进制码位一组，转换为4个6位二进制码为一组（不足6位时地位补0）。3个8位二进制码和4个6位二进制码长度都是24位。
 * <li>2. 对获得的4个6位二进制码补位，每个6位二进制码添加两位高位0，组成4个8位二进制码。
 * <li>3. 将获得的4个8位二进制码转换为4个十进制码。
 * <li>4. 将获得的十进制码转换为Base64字符表中对应的字符。
 * <P>
 * 例如：
 * <li>字符串“A”,
 * <li>ASCII码 65,
 * <li>二进制码 01000001,
 * <li>4个6位二进制码 010000 010000,
 * <li>4个8位二进制码 00010000 00010000
 * <li>十进制码 16 16
 * <li>字符表映射码 Q Q = =
 * <p>
...
class MyBase64Encoder implements IBase64 {
     ...
     * base64码表
     */
    public static final byte[] base = { 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d,
...
interface IBase64 {
    ...
    /**
     * 解码 将编码后的字符串数组的最后2个字节码还原为原来的字节码 假如数组末尾剩下2个字节： 将倒数第2个字节的前后6位(00xxxxxx) 和倒数第一个字节的后2位(000000yy) 还原为原来的编码（xxxxxxyy）
     * 假如数组末尾剩下3个字节： 将倒数第2个字节的前后4位(0000xxxx) 和倒数第一个字节的后4位(0000yyyy) 还原为原来的编码（xxxxyyyy）
     * 
     * @paramlast_b倒数第2个字节
     * @paramnext_b倒数第1个字节
    ...
    /**
     * 假如字符长度%3！=0，使用此方法编码末尾字符 假如b=xxxxyyyy 假如末尾字节个数等于1： 将这个字节的前6位作为一个字节(00xxxxyy) 将这个字节的后6位作为一个字节(00xxyyyy)
     * 假如末尾字节个数等于2： 将这个字节的后6位作为一个字节(00xxyyyy)
     * 
     * @paramb末尾的字符的字节码
    ...
    /**
     * 编码 假如last_b=xxxxyyyynext_b=kkkkffff 将3个字节中的第1个字节的最后2位（000000yy） 和第2个字节的前4位（kkkk0000）编码为（00yykkkk）
     *
    ...
    /**
     * 编码 假如last_b=xxxxyyyynext_b=kkkkffff 将3个字节中的第2个字节的最后4位（0000yyyy） 和第4个字节的前2位（kk000000）编码为（00yyyykk）
     *
     *
